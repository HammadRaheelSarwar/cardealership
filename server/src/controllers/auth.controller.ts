...
async function session(
  ...
  user: any,
  statusCode = 200,
  authSession?: { access_token: string; refresh_token: string } | null,
) {
...
export async function register(
  ...
  try {
    ...
    const auth = await createAuthClient().auth.signInWithPassword({
      email,
      password,
    });
    ...
    await session(req, res, profile, 201, auth.data.session);
  } catch (error) {
    next(error);
  }
}
export async function login(
  ...
  try {
    ...
    await session(req, res, user, 200, data.session);
  } catch (error) {
    next(error);
  }
}
